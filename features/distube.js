        ...
        if(message.content.startsWith("op"))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        distube.stop(process.env.guildid);
                        message.channel.send("Stopped the queue!!!");
                    }
                    else if(command==="pause")
                    {
                        distube.pause(process.env.guildid);
                        message.channel.send("Music is paused!!!");
                    }
                    else if(command==="resume")
                    {
                        distube.resume(process.env.guildid);
                    ...
                    {
                        distube.shuffle(process.env.guildid);
                        message.channel.send("Queue is been shuffled!!!");
                    }
                    else if(command === "skip")
                    {
                        let stat=0;
                        distube.skip(process.env.guildid).catch((err)=>
                        {
